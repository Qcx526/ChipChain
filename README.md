...
- Phase 9B1 ARM32 QEMU passive observer 离线实现：probe、strict raw parser、adapter 与 safe runner
- dumb TCG plugin source 和 owned synthetic bare-metal ELF（真实 QEMU 验证当前受环境阻塞）
...
真实 QEMU observer 的本机编译/加载验证、Type III hardware→software propagation verification
或 API。Phase 9B1 已完成离线合同与实现，但当前缺少 QEMU、supported plugin build
environment 和 plugin headers，绝不
...
Phase 9B1 的真实 smoke test 是显式可选能力，不会自动下载 QEMU、编译器或 headers：
