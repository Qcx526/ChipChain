"""Synthetic reasoning fixtures: independent hand-built hardware-side inputs only."""
...
from chipchain.reasoning.trigger_candidate import ReasoningProviderProfile
...
@pytest.fixture
def context_inputs(hardware_case):
    raw = hardware_case.si()
    # Deliberately omit the middle instruction's symbol: no label propagation.
    data = hardware_case.elf(symbols=[("_p0", 0x200, 1), ("_s0", 0x208, 1)])
    elf = parse_hardware_test_elf(data)
    isa, rtl = hardware_case.csv(), hardware_case.rtl(state=2)
    scope = AlignmentScope(isa_source=isa.source, rtl_source=rtl.source,
        common_start_pc={"value": "0x200"}, comparable_fields=tuple(ComparableField))
    result = align_common_program(scope, isa, rtl)
    divergence = DivergenceObservation(aligned_pair=result.aligned_pairs[1], field=ComparableField.MSTATUS)
    fragment = hardware_case.fragment(raw)
    source = TriggerSourceContext(source_kind="synthetic_fixture", artifact=fragment.source.artifact,
        hardware_target=fragment.source.hardware_target, producer_profile_id=fragment.source.producer_profile_id)
    return dict(raw_si=raw, elf=elf, elf_bytes=data, alignment=result, divergence=divergence,
        source=source, before=1, after=1)


@pytest.fixture
def context(context_inputs):
    return build_trigger_candidate_context(**context_inputs)


@pytest.fixture
def provider_profile():
    return ReasoningProviderProfile(provider_profile_id="synthetic-fake-v1", model_id="fake-model-v1")


@pytest.fixture
def proposal_payload(context):
    ref = next(ref for ref in context.fact_index() if ref.kind == "ISA_OBSERVATION").model_dump(mode="json")
    return {
        "schema_version": "v2_model_trigger_candidate_proposal_v1", "disposition": "PROPOSE",
        "proposed_preconditions": [{"local_id": "state-a", "kind": "register_state",
            "register_ref": {"register_class": "system", "namespace": "synthetic-registers", "name": "control"},
            "width_bits": 8, "value": "0x2"}],
        "proposed_steps": [
            {"local_id": "step-a", "kind": "instruction", "mnemonic": "synthetic.op"},
            {"local_id": "step-b", "kind": "register_access", "register_ref": {
                "register_class": "gpr", "namespace": "synthetic-registers", "name": "r1"}, "access": "write"},
        ],
        "proposed_order": [{"local_id": "order-a", "kind": "required_precedes", "before_id": "step-a", "after_id": "step-b"}],
        "supports": [{"proposal_id": name, "evidence_refs": [ref]} for name in ("state-a", "step-a", "step-b", "order-a")],
        "rationale_atoms": [{"statement_id": "synthetic-reasoning", "text": "A benign proposed condition, not an established trigger.",
            "supporting_evidence_refs": [ref]}],
        "unresolved_condition_ids": [c.id for c in context.unresolved_conditions],
    }


@pytest.fixture
def abstain_payload(context):
    return {"schema_version": "v2_model_trigger_candidate_proposal_v1", "disposition": "ABSTAIN",
        "proposed_preconditions": [], "proposed_steps": [], "proposed_order": [], "supports": [],
        "rationale_atoms": [], "unresolved_condition_ids": [c.id for c in context.unresolved_conditions]}
